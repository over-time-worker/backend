...
public class ProductInfoServiceImpl implements ProductInfoService {
    ...
    public void create(CreateProductInfoRequestDto createProductInfoRequestDto) {
        ...
        //1. 상품명 중복 검사
        productInfoRepository.findByProductNmae(createProductInfoRequestDto.getProductName()).ifPresent(
                productInfo -> {
        ...
        //3. 상품 정보 - 생산 업체 연결
        ProductInfo productInfo = CreateProductInfoRequestDto.toEntity(createProductInfoRequestDto,producer);
        productInfo.updateCreateData(1L); //TODO:: AuditAware후 삭제

        producer.getProductInfos().add(productInfo);
