...
import static com.owlexpress.consumer.domain.entity.QConsumer.consumer;
...
public class ConsumerQueryDslRepositoryImpl implements ConsumerQueryDslRepository {
    ...
    @Override
    public Page<Consumer> searchConsumer(
            String sort,
            String q,
            String orderBy,
        ...
        if (q != null && !q.isEmpty()) {
            builder.and(consumer.companyName.containsIgnoreCase(q));
        }
        ...
        OrderSpecifier<?> orderSpecifier = switch (sort) {
            case "name" -> new OrderSpecifier<>(order, consumer.companyName);
            case "address" -> new OrderSpecifier<>(order, consumer.companyAddress);
            default -> new OrderSpecifier<>(order, consumer.consumerId); // 기본 정렬 기준: productId
        };

        // QueryDSL을 사용하여 검색 및 페이징 처리
        List<Consumer> results = queryFactory
                .selectFrom(consumer) // producer 테이블에서 조회
