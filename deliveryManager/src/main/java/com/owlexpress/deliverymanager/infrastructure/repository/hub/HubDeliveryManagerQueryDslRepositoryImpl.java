...
public class HubDeliveryManagerQueryDslRepositoryImpl implements HubDeliveryManagerQueryDslRepository {
    ...
    public Page<HubDeliveryManager> searchProducer(
        ...
        if (q != null && !q.isEmpty()) {
            builder.and(hubDeliveryManager.userName.containsIgnoreCase(q));
        }
        ...
        OrderSpecifier<?> orderSpecifier = switch (sort) {
            case "name" -> new OrderSpecifier<>(order, hubDeliveryManager.userName);
            case "assignNumber" -> new OrderSpecifier<>(order, hubDeliveryManager.assignNumber);
            default -> new OrderSpecifier<>(order, hubDeliveryManager.hubDeliveryManagerId); // 기본 정렬 기준: productId
        };

        // QueryDSL을 사용하여 검색 및 페이징 처리
        List<HubDeliveryManager> results = queryFactory
                .selectFrom(hubDeliveryManager) // producer 테이블에서 조회
